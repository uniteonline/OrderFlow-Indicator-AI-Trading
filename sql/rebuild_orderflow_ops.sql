...
-- Drops runtime/config schemas in current database, then recreates all objects.
-- Usage:
...
BEGIN
    ...
    IF current_db <> 'orderflow_ops' THEN
        RAISE EXCEPTION
            'Refused to rebuild database "%". Expected "orderflow_ops".',
            current_db;
    END IF;
END
$$;
...
SELECT ops.ensure_outbox_event_partitions(7, 2);

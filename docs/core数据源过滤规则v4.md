...
### 4.7 新增 `atr_context`：为 entry 提供当前时间切片的 ATR 概览

**背景**：

`stage1 scan` 输入中显式包含 `atr14 / atr14_pct`，但 `stage2 entry_core` 之前没有等价的 ATR 概览结构。  
这会导致 entry 阶段虽然能看到少量 ATR 派生值（例如事件的 `price_distance_atr`），但看不到与 `scan` 同风格的当前 `15m / 4h / 1d` 波动摘要。

**设计目标**：

- 在 `entry_core` 中新增一个轻量的 `atr_context`
- 命名尽量与 `scan.path_newest_to_oldest.*.summary` 保持一致
- **必须使用 stage2 entry 当前分钟切片的数据重新计算**
- **不能复用 stage1 scan 的旧 ATR 结果**

也就是说，`entry_core` 中的 ATR 代表的是 **当前 entry 时点** 的波动状态，而不是前一个 `scan` 桶的 ATR。

**来源与计算方式**：

- 数据来源：`entry_core` 自己保留的 `kline_history`
- 窗口：`15m / 4h / 1d`
- ATR 算法：与 `scan` 相同，基于对应窗口 futures closed bars 的 `ATR14`
- `atr14_pct = atr14 / latest_close * 100`

**输出位置**：

```text
indicators.atr_context.payload.latest_15m_detail.summary
indicators.atr_context.payload.latest_4h_context.summary
indicators.atr_context.payload.latest_1d_background.summary
```
...
| 字段 | 含义 |
|------|------|
| `bars_count` | 当前切片中用于概览的已闭合 bars 数量 |
| `oldest_open` | 当前切片起点 bar 的开盘价 |
| `latest_close` | 当前切片终点 bar 的收盘价 |
| `atr14` | 当前窗口 ATR14 |
| `atr14_pct` | `atr14 / latest_close * 100` |
| `net_change_pct` | 当前切片起点到终点的净涨跌幅 |
| `range_high` | 当前切片高点 |
| `range_low` | 当前切片低点 |
| `range_pct` | 当前切片整体振幅占比 |
...
**为什么不直接透传 scan 的完整 path 结构**：

- `entry_core` 只需要当前时点的 ATR 参考，不需要完整的 `scan.path_newest_to_oldest`
- 轻量 `summary` 已足够支持：
  - entry / SL / TP 的波动尺度判断
  - prompt 中对 range、target、execution quality 的推理

**为什么不只保留 ATR 派生值**：

目前 `entry_core` 已会在局部字段里使用 ATR 派生值，例如：

- `events_summary.*.price_distance_atr`

但这些派生值不能替代当前 `15m / 4h / 1d` 的 ATR 全局概览。  
`atr_context` 的作用是补充“当前切片的波动尺度背景”，而不是重复已有事件距离字段。

**预期影响**：

- 体积增加很小（轻量 summary 级别）
- 能让 `entry` 在当前时间切片下直接引用 `15m / 4h / 1d` ATR，而不必依赖 `stage1 scan`
...
| 4.7 新增 atr_context | 轻微增加 | 当前 entry 时点的 15m/4h/1d ATR 概览 |
| **v4 合计新增节省** | **~65KB** | |
...
atr_ctx = indicators["atr_context"]["payload"]
assert atr_ctx["latest_15m_detail"]["summary"]["atr14"] is not None
assert atr_ctx["latest_4h_context"]["summary"]["atr14"] is not None
assert atr_ctx["latest_1d_background"]["summary"]["atr14"] is not None
assert atr_ctx["latest_15m_detail"]["summary"]["atr14_pct"] is not None

use super::core_shared::{
    self, copy_fields, filter_avwap, filter_cvd_pack_entry_v3, filter_divergence,
...
const ENTRY_EVENT_SUMMARY_FIELDS: &[(&str, &str)] = &[
...
pub(super) fn filter_indicators(
    ...
    core_shared::insert_filtered_indicator(
        ...
        source,
        "high_volume_pulse",
        filter_high_volume_pulse_entry_v4,
    );
    ...
    core_shared::insert_filtered_indicator(
        ...
        source,
        "tpo_market_profile",
        filter_tpo_market_profile_entry_v4,
    );
    core_shared::insert_filtered_indicator(
        &mut indicators,
        source,
        "price_volume_structure",
        filter_price_volume_structure_entry_v4,
    );
...
fn filter_tpo_market_profile_entry_v4(payload: &Value) -> Value {
...
fn filter_fvg_entry_v4(payload: &Value) -> Value {
    let mut filtered = filter_fvg(payload, CORE_WINDOWS_WITH_3D, 6);
...
fn filter_footprint_entry_v4(payload: &Value) -> Value {
    let mut filtered = filter_footprint_entry_v3(payload);
    if let Some(by_window) = filtered.get_mut("by_window").and_then(Value::as_object_mut) {
...
fn filter_ema_trend_regime_entry_v4(payload: &Value) -> Value {
...
fn filter_cvd_pack_entry_v4(payload: &Value) -> Value {
    prune_nulls(filter_cvd_pack_entry_v3(payload, CVD_LIMITS))
}

fn filter_divergence_entry_v4(payload: &Value) -> Value {
...
fn filter_vpin_entry_v4(payload: &Value) -> Value {
...
fn filter_high_volume_pulse_entry_v4(payload: &Value) -> Value {
    let Some(payload) = payload.as_object() else {
        return Value::Null;
    };
    let mut result = Map::new();
...
fn build_events_summary(
    ...
    for (indicator_code, output_key) in ENTRY_EVENT_SUMMARY_FIELDS {
...
fn sanitize_fvg_object_field_v4(target: &mut Map<String, Value>, field: &str) {
...
fn sanitize_fvg_array_field_v4(target: &mut Map<String, Value>, field: &str) {
...
fn sanitize_fvg_value_v4(value: Value) -> Value {

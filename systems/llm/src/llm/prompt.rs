...
pub fn user_prompt_prefix(
    ...
    if pending_order_mode {
        ...
        "You are in entry finalize mode. Analyze the current stage-2 order-flow snapshot together with the STAGE_1_MARKET_SCAN_JSON. Treat the current stage-2 prompt input as the authoritative execution-time dataset, and use the stage-1 scan as earlier structural context. Stage-1 trend directions (15m, 4h, 1d) are an earlier structural read, not a trading instruction. If your decision differs, explain whether current stage-2 evidence shows continuation, resistance, or transition. Return only the final entry decision JSON.\n\n"
    }
}

...
pub fn user_prompt_prefix(
    ...
    if pending_order_mode {
        "You are in pending-order management mode. Analyze the latest indicators plus the live pending maker-order context from `trading_state` and `management_snapshot` (especially `management_snapshot.pending_order`, `management_snapshot.position_context.entry_context`, and the previous entry reason). Also review the STAGE_1_MARKET_SCAN_JSON provided above for broader market context. Decide whether to keep the pending order, cancel it, or modify the maker entry / TP / SL. Return only a pending-order management decision JSON.\n\n"
    } else if management_mode {
        "You are in management mode. Analyze latest indicators plus current position/order context from `trading_state` and `management_snapshot` (especially `context_state`, leverage, direction, position quantity, open orders, and last management reason). Also review the STAGE_1_MARKET_SCAN_JSON provided above for broader market context. IMPORTANT: `management_snapshot.positions[].current_tp_price` and `current_sl_price` are the ACTUAL placed TP/SL order trigger prices on the exchange (sourced directly from Binance open orders — not model estimates). Use them as baselines for all MODIFY_TPSL decisions: HC-6 requires new values to differ from these actual current prices; HC-9 requires new_sl to be tighter (more favorable) than current_sl_price. Return only a management decision JSON.\n\n"
    } else {
        "You are in entry finalize mode. Analyze the order-flow snapshot together with the STAGE_1_MARKET_SCAN_JSON and return only the final entry decision JSON.\n\n"
    }
}

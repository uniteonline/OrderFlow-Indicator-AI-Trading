...
use crate::exchange::binance::ws::{futures_streams, reconnect};
use crate::normalize::{
    self, kline_normalizer, mark_price_normalizer, trade_normalizer, Market, NormalizedMdEvent,
};
...
// For futures burst rates (~1.5k-2k events/s), 10k queue depth is the early warning line.
const WS_PERSIST_QUEUE_CONGESTED_THRESHOLD: usize = 10_000;
...
pub async fn run(
    ...
    match checkpoints::load_checkpoint_seeds(&ctx.ops_db_pool, "futures", SYMBOL).await {
        ...
        Err(err) => {
            warn!(error = %err, market = "futures", "failed to load checkpoint seeds");
        }
    }
    ...
    .await
    {
        warn!(error = %err, market = "futures", "startup kline bootstrap failed");
    }
            ...
            tokio::select! {
                _ = stats_interval.tick() => {
                    for event in ws_preagg.flush_expired(Utc::now()) {
                        enqueue_persist_event(&persist_queue, "futures", event).await;
                    }
                    ...
                    if force_reconnect_after_event {
                        ...
                        Err(err) => {
                            ...
                            warn!(
                                ...
                                error = %err,
                                error_text = %err_text,
                                "futures websocket receive error"
                            );
                            break;
                        }
                    }
...
async fn handle_trade_gap(
    ...
    {
        // Spawn the gap-record write so it never stalls the WS receive loop
        // (same pattern as handle_depth_gap).
        {
            let ops_writer = Arc::clone(ops_writer);
...
async fn replay_trade_gap(
    ...
    let mut throttle = BackfillThrottle::new("futures", BACKFILL_TRADE_THROTTLE);
...
async fn handle_depth_gap(
    ...
    if let Some(gap) = gap_detector.detect_depth_gap(
        ...
        // Spawn the ops-db write so it never blocks the WS receive loop.
        // A slow/exhausted ops_db pool would otherwise cause the WS task to
        // suspend for up to acquire_timeout_secs, buffering 10-15 s of events
        // in the OS TCP socket and producing the "persist lag over 10s" burst.
        let ops_writer = Arc::clone(ops_writer);
        let market = event.market.clone();
        let symbol = event.symbol.clone();
        let stream_name = event.stream_name.clone();
        ...
        tokio::spawn(async move {
            if let Err(err) = ops_writer
                ...
                .await
            {
                // Use {:#} to print the full anyhow error chain including the
                // underlying sqlx / postgres error, not just the outermost context.
                warn!("insert depth gap failed: {:#}", err);
            }
        });

        let requested = depth_rebuilder::request_immediate_depth_reconcile(Market::Futures);
        if requested {
            info!(
                market = %event.market,
                symbol = %event.symbol,
...
async fn replay_kline_gap(
    ...
    let mut throttle = BackfillThrottle::new("futures", BACKFILL_KLINE_THROTTLE);
...
async fn bootstrap_missing_kline_history(
    ...
    if bootstrap_intervals.is_empty() {
        return Ok(());
    }
    ...
    for interval_code in bootstrap_intervals {
        let Some(step_ms) = interval_ms(interval_code) else {
            continue;
        };
        let end_open_ms = last_closed_open_time_ms(step_ms);
        let start_ms = end_open_ms
            .saturating_sub(step_ms.saturating_mul(KLINE_STARTUP_BOOTSTRAP_BARS.saturating_sub(1)));
        let rows = rest_client
            .fetch_klines(
                "futures",
                SYMBOL,
        ...
        let mut db_events = Vec::with_capacity(rows.len().saturating_mul(2));
        let mut written = 0i64;
        let mut latest_open_time_ms: Option<i64> = None;
        let mut checkpoint_seed: Option<(NormalizedMdEvent, checkpoints::CheckpointUpdate)> = None;

        for row in &rows {
            if row.open_time_ms < start_ms || row.open_time_ms > end_open_ms {
                continue;
            }
            ...
            checkpoint_seed = Some((event.clone(), checkpoints::build_checkpoint(&event)));
            db_events.push(event.clone());

            if interval_code == "1d" {
                match kline_3d_aggregator.lock().await.on_kline(&event) {
                    Ok(Some(derived_3d)) => db_events.push(derived_3d),
                    Ok(None) => {}
                    Err(err) => {
                        warn!(error = %err, "update 3d kline aggregator failed during startup bootstrap")
                    }
                }
            }
            latest_open_time_ms = Some(row.open_time_ms);
            written += 1;
        }

        if !db_events.is_empty() {
            db_writer.write_md_events_batch(&db_events).await?;
        }

        if let Some((event, checkpoint)) = checkpoint_seed {
            ops_writer
                .upsert_checkpoint(
                    &event.market,
                    &event.symbol,
                    ...
                    checkpoint.last_agg_trade_id,
                    checkpoint.checkpoint_json,
                )
                .await?;
        }

        if let Some(open_time_ms) = latest_open_time_ms {
            last_closed_kline_open_ms.insert(interval_code.to_string(), open_time_ms);
        }
        ...
        ops_writer
            .insert_backfill_job_run(
                ...
                json!({
                    ...
                    "start_ms": start_ms,
                    "end_ms": end_open_ms,
                    "bars_target": KLINE_STARTUP_BOOTSTRAP_BARS
                }),
                Some(written),
                "success",
                None,
...
async fn reconcile_after_ws_reconnect(
    ...
    for interval_code in KLINE_INTERVALS {
        ...
        let end_ms = last_closed_open_time_ms(step);
        ...
        let mut written = 0i64;
        let mut kline_throttle = BackfillThrottle::new("futures", BACKFILL_KLINE_THROTTLE);
    ...
    match rest_client.fetch_premium_index(SYMBOL).await {
        Ok(premium) => {
            ...
            if !try_enqueue_registered_persist_event(event.market.as_str(), event.clone()).await? {
                warn!(
                    market = event.market,
                    symbol = event.symbol,
                    msg_type = event.msg_type,
                    "registered persist queue unavailable; skip direct premium index persist to avoid duplicate canonical funding aggregation"
                );
            }
            ops_writer
                .insert_backfill_job_run(
        ...
        Err(err) => {
            warn!(error = %err, "futures reconnect premiumIndex reconcile failed");
...
fn last_closed_open_time_ms(step_ms: i64) -> i64 {
    let now_ms = Utc::now().timestamp_millis();
    let current_open_ms = now_ms - now_ms.rem_euclid(step_ms);

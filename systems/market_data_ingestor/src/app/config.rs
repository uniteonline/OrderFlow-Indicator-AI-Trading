...
pub struct MarketDataConfig {
    #[serde(default = "default_market_data_symbol")]
    pub symbol: String,
}
...
impl Default for MarketDataConfig {
    fn default() -> Self {
        Self {
            symbol: default_market_data_symbol(),
        }
    }
}
...
pub fn load_config(path: &str) -> Result<RootConfig> {
    ...
    let symbol = extract_global_symbol(&doc)
        .ok_or_else(|| anyhow!("config.instrument.symbol or market_data.symbol is required"))?;
    ensure_yaml_string_path(&mut doc, &["market_data", "symbol"], &symbol);
...
fn validate_config(cfg: &RootConfig) -> Result<()> {
    ...
    if cfg.market_data.symbol.trim().is_empty() {
        return Err(anyhow!("config.market_data.symbol is empty"));
    }
...
fn default_market_data_symbol() -> String {
    String::new()
}
